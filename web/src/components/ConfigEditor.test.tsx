...
function stubFetch() {
  const fetchMock = vi.fn().mockImplementation((input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url === "/api/config/design" && method === "PUT") {
      ...
      return Promise.resolve(jsonResponse(200, { path: "/tmp/design.yaml", ...body }));
    }
...
describe("ConfigEditor", () => {
  ...
  it("disables Save while the Raw view has invalid JSON, and re-enables once it's valid again", async () => {
    stubFetch();
    render(<ConfigEditor />);
    await screen.findByDisplayValue("0.1");
    fireEvent.click(screen.getByRole("button", { name: "Raw" }));
  ...
  it("saves the Form-edited value via PUT and refetches", async () => {
